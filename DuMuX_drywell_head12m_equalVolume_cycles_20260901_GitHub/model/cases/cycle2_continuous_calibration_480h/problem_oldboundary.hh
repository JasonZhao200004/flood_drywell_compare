...
private:
    ...
    {
        // --------------------------------------------------------------
        // Equal-volume two-cycle schedule
        //
        // Cycle 1:
        //   0 -> 176.490541 h       wetting
        //   176.490541 -> 240 h     drainage
        //
        // Cycle 2:
        //   240 -> 240.5 h          30-min startup ramp
        //   240.5 -> Cycle2WetEnd   full 12-m wetting
        //   Cycle2WetEnd -> 480 h   drainage
        //
        // Cycle2WetEndHours is supplied at runtime:
        //
        // Calibration:
        //   -Problem.Cycle2WetEndHours 480
        //
        // Formal:
        //   -Problem.Cycle2WetEndHours <matched absolute time>
        // --------------------------------------------------------------

        constexpr Scalar cycle1WetEndHours = 176.490541;
        constexpr Scalar cycle2StartHours = 240.0;
        constexpr Scalar cycle2RampEndHours = 240.5;

        const Scalar cycle2WetEndHours =
            getParam<Scalar>("Problem.Cycle2WetEndHours", 480.0);

        const Scalar timeHours = timeSeconds/3600.0;

        // Cycle 1 wetting
        if (timeHours <= cycle1WetEndHours)
            return drywellInjectionHeadCm_;

        // Cycle 1 drainage
        if (timeHours < cycle2StartHours)
            return Scalar(-25.0);

        // Cycle 2 startup: same 30-min operational ramp as Cycle 1.
        if (timeHours < cycle2RampEndHours)
        {
            const Scalar f =
                (timeHours - cycle2StartHours)
                /(cycle2RampEndHours - cycle2StartHours);

            return Scalar(-25.0)
                + f*(drywellInjectionHeadCm_ + Scalar(25.0));
        }

        // Cycle 2 full wetting
        if (timeHours <= cycle2WetEndHours)
            return drywellInjectionHeadCm_;
