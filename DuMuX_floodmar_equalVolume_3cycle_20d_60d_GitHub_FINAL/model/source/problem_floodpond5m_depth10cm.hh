...
public:
    ...
    , cycle1ShutdownStartHours_(
        getParam<Scalar>("Problem.Cycle1ShutdownStartHours", 91.3)
      )
    , cycle2ShutdownStartHours_(
        getParam<Scalar>("Problem.Cycle2ShutdownStartHours", 576.0)
      )
    , cycle3ShutdownStartHours_(
        getParam<Scalar>("Problem.Cycle3ShutdownStartHours", 1056.0)
      )
    ...
    {
        const std::array<Scalar, 12> eventHours = {
            0.0,
            cycle1ShutdownStartHours_,
            cycle1ShutdownStartHours_ + 5.0,
            480.0,
            485.0,
            cycle2ShutdownStartHours_,
            cycle2ShutdownStartHours_ + 5.0,
            960.0,
            965.0,
            cycle3ShutdownStartHours_,
            cycle3ShutdownStartHours_ + 5.0,
            1440.0
        };
...
private:
    ...
    Scalar drywellHeadCm_(Scalar timeSeconds) const
    {
        ...
        // ============================================================
        // Flood-MAR equal-dose experiment
        //
        // Common cycle length = 480 h = 20 d
        //
        // Cycle 1:
        //   0 -> C1Down             : +5 cm
        //   C1Down -> C1Down+5 h    : smooth +5 -> 0 cm
        //   then dry to 480 h
        //
        // Cycle 2:
        //   480 -> 485 h            : smooth 0 -> +5 cm
        //   485 -> C2Down           : +5 cm
        //   C2Down -> C2Down+5 h    : smooth +5 -> 0 cm
        //   then dry to 960 h
        //
        // Cycle 3:
        //   960 -> 965 h            : smooth 0 -> +5 cm
        //   965 -> C3Down           : +5 cm
        //   C3Down -> C3Down+5 h    : smooth +5 -> 0 cm
        //   then dry to 1440 h
        //
        // Global startup ramp at t=0 remains controlled by
        // Problem.DrywellStartupRampTime (=1800 s production value).
        //
        // Pond boundary physics, 0.5-cm dryout smoothing,
        // O2 transport, dispersion, phase guard, etc. are unchanged.
        // ============================================================
        ...
        if (timeHours <= cycle1ShutdownStartHours_)
            return injectionHeadCm;

        if (timeHours <= cycle1ShutdownStartHours_ + 5.0)
            return smoothDown(cycle1ShutdownStartHours_);
        ...
        if (timeHours <= cycle2ShutdownStartHours_)
            return injectionHeadCm;

        if (timeHours <= cycle2ShutdownStartHours_ + 5.0)
            return smoothDown(cycle2ShutdownStartHours_);
        ...
        if (timeHours <= 965.0)
            return smoothUp(960.0);

        if (timeHours <= cycle3ShutdownStartHours_)
            return injectionHeadCm;

        if (timeHours <= cycle3ShutdownStartHours_ + 5.0)
            return smoothDown(cycle3ShutdownStartHours_);
    ...
    Scalar cycle1ShutdownStartHours_;
    Scalar cycle2ShutdownStartHours_;
    Scalar cycle3ShutdownStartHours_;
