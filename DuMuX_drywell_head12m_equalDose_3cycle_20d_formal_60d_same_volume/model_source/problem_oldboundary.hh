...
public:
    ...
    {
        static constexpr std::array<Scalar, 6> eventHours = {
            176.490541000,
            480.000000000,
            725.850601953,
            960.000000000,
...
private:
    ...
    {
        // ================================================================
        // FINAL 60-DAY / 3-CYCLE EQUAL-VOLUME DRYWELL MAR EXPERIMENT
        //
        // Target gross injection volume:
        //     V* = 3769.844 m3 per cycle
        //
        // Common cycle duration:
        //     20 days = 480 h
        //
        // Cycle 1:
        //     wet   0.000000000 -- 176.490541000 h
        //     drain 176.490541000 -- 480.000000000 h
        //
        // Cycle 2:
        //     wet   480.000000000 -- 725.850601953 h
        //     drain 725.850601953 -- 960.000000000 h
        //
        // Cycle 3:
        //     wet   960.000000000 -- 1210.359144226 h
        //     drain 1210.359144226 -- 1440.000000000 h
        //
        // Injection head = 1200 cm
        // Drainage head  = -25 cm
        //
        // Startup ramp applies ONLY at global simulation startup.
        // C2 and C3 switches are direct operational transitions.
        // ================================================================

        constexpr Scalar c1WetEndHours = 176.490541000;
        constexpr Scalar c2StartHours  = 480.000000000;
        constexpr Scalar c2WetEndHours = 725.850601953;
        constexpr Scalar c3StartHours  = 960.000000000;
        constexpr Scalar c3WetEndHours = 1210.359144226;

        const Scalar timeHours = timeSeconds/3600.0;

        // Cycle 1 wetting
        if (timeHours <= c1WetEndHours)
            return drywellInjectionHeadCm_;

        // Cycle 1 drainage
        if (timeHours < c2StartHours)
            return Scalar(-25.0);

        // Cycle 2 wetting
        if (timeHours <= c2WetEndHours)
            return drywellInjectionHeadCm_;

        // Cycle 2 drainage
        if (timeHours < c3StartHours)
            return Scalar(-25.0);

        // Cycle 3 wetting
        if (timeHours <= c3WetEndHours)
            return drywellInjectionHeadCm_;

        // Cycle 3 drainage through day 60
        return Scalar(-25.0);
    }
