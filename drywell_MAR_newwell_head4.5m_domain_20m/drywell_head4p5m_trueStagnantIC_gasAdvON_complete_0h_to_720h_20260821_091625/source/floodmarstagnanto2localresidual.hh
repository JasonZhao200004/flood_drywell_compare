...
#ifndef DUMUX_FLOODMAR_STAGNANT_O2_LOCAL_RESIDUAL_HH
#define DUMUX_FLOODMAR_STAGNANT_O2_LOCAL_RESIDUAL_HH
...
class FloodMarStagnantO2LocalResidual
                            ...
                            const ElementFluxVariablesCache& elemFluxVarsCache) const
    {
        ...
        for (int phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx)
        {
            ...
            for (int compIdx = 0; compIdx < numComponents; ++compIdx)
            {
                ...
                // Advective component fluxes.  For the HYDRUS-overlap
                // diagnostic, O2 is stagnant in the gas phase.  Its suppressed
                // advective molar flux is assigned to inert N2 so that the bulk
                // gas molar flux, pressure response, and venting remain unchanged.
                if (eqIdx != replaceCompEqIdx)
                {
                    const bool isGasOxygen =
                        phaseIdx == FluidSystem::gasPhaseIdx
                        && compIdx == FluidSystem::O2Idx;

                    if (!isGasOxygen)
                        flux[eqIdx] += fluxVars.advectiveFlux(phaseIdx, upwindTerm);

                    if (phaseIdx == FluidSystem::gasPhaseIdx
                        && compIdx == FluidSystem::N2Idx)
                    {
                        const auto oxygenUpwindTerm =
                            [&massOrMoleDensity, &massOrMoleFraction, phaseIdx]
                            (const auto& volVars)
                            {
                        ...
                        flux[eqIdx] +=
                            fluxVars.advectiveFlux(phaseIdx, oxygenUpwindTerm);
                    }
                }
