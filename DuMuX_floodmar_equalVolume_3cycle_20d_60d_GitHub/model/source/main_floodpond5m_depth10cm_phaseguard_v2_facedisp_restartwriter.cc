...
#include "properties_floodpond5m_depth10cm_phaseguard_v2_facedisp.hh"
...
int main(int argc, char** argv)
{
    ...
    // Optional exact primary-variable restart snapshot at the accepted final state.
    const bool writeRestartAtEnd =
        getParam<bool>("Restart.WriteAtEnd", false);

    if (writeRestartAtEnd && mpiHelper.rank() == 0)
    {
        const std::string restartOutputFile =
            getParam<std::string>("Restart.OutputFile");

        std::ofstream restartOut(restartOutputFile);
        if (!restartOut)
            throw std::runtime_error(
                "Could not open FloodMAR restart output file: "
                + restartOutputFile
            );

        restartOut << std::setprecision(17);
        restartOut
            << "FLOODMAR_RESTART_V1 "
            << x.size() << " "
            << timeLoop->time() << "\n";
        ...
        restartOut.close();
        std::cout
            << "Wrote FloodMAR restart: " << restartOutputFile
            << "\nRestart time = " << timeLoop->time()
            << " s (" << timeLoop->time()/3600.0 << " h)"
            << std::endl;
    }
