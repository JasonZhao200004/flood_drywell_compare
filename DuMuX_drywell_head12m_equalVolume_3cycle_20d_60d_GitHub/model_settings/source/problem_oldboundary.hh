...
public:
    ...
    Scalar nextBoundaryEventTime(Scalar currentTimeSeconds) const
    {
        ...
        const std::array<Scalar, 9> eventHours = {
            0.5,
            c1,
            480.0,
            480.5,
        ...
        Scalar nextEventTime =
            std::numeric_limits<Scalar>::infinity();

        if (
            drywellStartupRampTime_
            > currentTimeSeconds + 1.0e-8
        )
        {
            nextEventTime =
                drywellStartupRampTime_;
        ...
        for (
            const Scalar eventHour :
            eventHours
        )
        {
            const Scalar eventTime =
                eventHour * 3600.0;
...
private:
    ...
    Scalar drywellHeadCm_(Scalar timeSeconds) const
    {
        ...
        // Cycle 1: 0-480 h
        //
        // Existing startupFactor in neumann()
        // ramps the initial positive head over 0.5 h.
        // ============================================================
