...
public:
                        ...
                        const SubControlVolumeFace& scvf) const
    {
        ...
        else if (isVariHeadBoundary_(globalPos))
        {
            ...
            // Represent the moving water surface continuously on each
            // vertical boundary face. Previously a complete face switched
            // abruptly from dry to wet when its integration point was
            // crossed. That discontinuity can trap the phase switch at one
            // degree of freedom even as dt approaches zero.
            //
            // For a partially submerged face, use the submerged fraction as
            // an area weight and evaluate hydrostatic pressure at the centre
            // of its wetted part. For a fully submerged face this reduces
            // exactly to the previous midpoint formula.
            Scalar wettedFraction = 0.0;
            Scalar fluxEvaluationElevation = globalPos[1];

            const Scalar waterSurfaceElevation =
                drywellBottomElevation_ + rampedDrywellHeadCm/100.0;

            const bool isVerticalDrywellWall =
                std::abs(globalPos[0] - drywellRadius_) < eps_
                && globalPos[1] > drywellBottomElevation_ + eps_;

            if (rampedDrywellHeadCm > 0.0 && isVerticalDrywellWall)
            {
                const Scalar faceLength = scvf.area();
                const Scalar faceBottomElevation =
                    globalPos[1] - 0.5*faceLength;

                wettedFraction = std::clamp(
                    (waterSurfaceElevation - faceBottomElevation)/faceLength,
                    Scalar(0.0),
                    Scalar(1.0)
                );

                fluxEvaluationElevation =
                    faceBottomElevation
                    + 0.5*wettedFraction*faceLength;
            }
            else if (rampedDrywellHeadCm > 0.0)
            {
                // The horizontal drywell bottom is fully wetted for any
                // positive water depth.
                wettedFraction = 1.0;
            }

            // HYDRUS Var H-1 is the pressure head at the lowest point of
            // the drywell (y = 14 m). Hydrostatic head decreases by one metre
            // per metre of elevation along the submerged drywell wall.
            const Scalar localDrywellHeadCm =
                rampedDrywellHeadCm
                - (fluxEvaluationElevation - drywellBottomElevation_)*100.0;
            ...
            // pressure head is negative". Rainfall and evaporation are both
            // zero. The exposed fraction therefore has no imposed liquid
            // flux, while the submerged fraction is weighted continuously.
            const Scalar liquidVolumeFlux =
                wettedFraction > 0.0
                ? wettedFraction*permeability*mobility
