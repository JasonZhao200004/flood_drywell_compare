...
#ifndef DUMUX_FLOODMAR_HYDRUSMATCH_STAGNANT_O2_LOCAL_RESIDUAL_HH
#define DUMUX_FLOODMAR_HYDRUSMATCH_STAGNANT_O2_LOCAL_RESIDUAL_HH
...
class FloodMarHydrusMatchStagnantO2LocalResidual
                            ...
                            const ElementFluxVariablesCache& elemFluxVarsCache) const
    {
        ...
        for (int phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx)
        {
            ...
            for (int compIdx = 0; compIdx < numComponents; ++compIdx)
            {
                ...
                // HYDRUS match: O2 does not advect with the Darcy gas flux.
                if (eqIdx != replaceCompEqIdx)
                {
                        ...
                        const auto oxygenUpwindTerm =
                            ...
                            (const auto& volVars)
                            {
                                return massOrMoleDensity(volVars, phaseIdx)
                                       *massOrMoleFraction(volVars, phaseIdx, FluidSystem::O2Idx)
                                       *volVars.mobility(phaseIdx);
                            };
                        flux[eqIdx] += fluxVars.advectiveFlux(phaseIdx, oxygenUpwindTerm);
                    }
                }
