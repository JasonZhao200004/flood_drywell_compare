...
public:
    ...
    , pondStartupRampTime_(
        getParam<Scalar>("Problem.PondStartupRampTime", 3600.0)
      )
      ...
      )
    {
        ...
        std::cout << "Flood-MAR axisymmetric water-N2-O2 model\n"
                  ...
                  << "Initial O2 profile = "
                  << initialOxygenProfileFile_ << "\n"
                  << "Flood-MAR pond: radius 3 m, excavation 0.5 m, "
                  << "VariHead on bottom and lowest 5 cm of rim\n"
                        ...
                        const SubControlVolumeFace& scvf) const
    {
        ...
        else if (isPondVariHead_(globalPos))
        {
            ...
            // The pond bottom (r = 0--3 m, y = 29.5 m) and only the
            // lowest 5 cm of the vertical rim share the HYDRUS VariHead
            // boundary. The specified head is spatially uniform on this
            // short boundary: +5 cm during recharge and -25 cm during
            // drainage. With zero rain/evaporation, negative head falls
            // back to zero imposed liquid flux.
            const Scalar localPondHeadCm = pondHeadCm_(time_);
            ...
            const Scalar startupFactor =
                pondStartupRampTime_ > 0.0
                ? std::clamp(time_/pondStartupRampTime_, Scalar(0.0), Scalar(1.0))
                : Scalar(1.0);
            ...
            // pond water level has no imposed liquid-water flux.
            const Scalar liquidVolumeFlux =
                localPondHeadCm > 0.0
                ? startupFactor*permeability*mobility
...
private:
    ...
    Scalar pondHeadCm_(Scalar timeSeconds) const
    {
        ...
        static constexpr std::array<Scalar, 12> heads = {
            5.0, 5.0, -25.0, -25.0, 5.0, 5.0,
            -25.0, -25.0, 5.0, 5.0, -25.0, -25.0
        };
    ...
    bool isPondVariHead_(const GlobalPosition& globalPos) const
    {
        ...
        const bool wettedRim =
            ...
            && y <= pondBottomElevation_ + 0.05 + eps_;

        return pondBottom || wettedRim;
    }

    static constexpr Scalar eps_ = 1.0e-6;
    static constexpr Scalar pondRadius_ = 3.0;
